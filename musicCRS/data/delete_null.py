...
conn = sqlite3.connect(
    "final_database.db"
)  # sostituisci con il percorso del tuo file DB
cursor = conn.cursor()

# Conta il numero di righe prima della cancellazione
cursor.execute(
    "SELECT COUNT(*) FROM music"
)  # sostituisci con il nome della tua tabella
total_rows_before = cursor.fetchone()[0]

# Esegui la cancellazione delle righe con acousticness null
cursor.execute(
    "DELETE FROM music WHERE acousticness IS NULL"
)  # sostituisci con il nome della tua tabella
conn.commit()

cursor.execute(
    "DELETE FROM music WHERE track_popularity IS NULL"
)  # sostituisci con il nome della tua tabella
conn.commit()
